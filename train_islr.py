...
from opt_islr import train_one_epoch_islr, evaluate_islr
import utils
...
def main(args, cfg):
    ...
    # gloss_tokenizer = (
    #     GlossTokenizer(config["gloss_tokenizer"]) if "gloss_tokenizer" in cfg else None
    # )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        train_results = train_one_epoch_islr(
            args,
            model,
            train_dataloader,
            optimizer,
        ...
        if results.get("acc", 0.0) > best_acc:
            ...
            checkpoint_paths = [output_dir / "best_islr_checkpoint.pth"]
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master(
                    {
                        "model": model.state_dict(),
