...
from utils.cslr import train_one_epoch, evaluate_fn
import utils


def get_args_parser():
    parser = argparse.ArgumentParser("MSCA-Net scripts", add_help=False)
...
def main(args, cfg):
    ...
    model = MSCA_Net(
        cfg=cfg["model"], device=device, num_classes=len(gloss_tokenizer) + 1
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("MSCA-Net scripts", parents=[get_args_parser()])
